...
tinymce.PluginManager.add('image', function(editor) {
	...
	function showDialog(imageList) {
		...
		function uploadFrom() {
			// mce_upload
			var buttonOK = "";
			$('#source_id').closest('div[role="application"]').find('button[role="presentation"]').each(function(i, el){
				if($(el).find('span').text() == "Ok"){
					buttonOK = $(el).attr('data-selector',	"disabled");
				}
			});

			var cb_img_url = function(json){
					var obj = $.parseJSON(json)
					console.log(obj.filelink);
					$("#source_id").find("input").val(obj.filelink);
					$(".mce-primary").removeClass("disabled");
					$("#spin_container").hide();
					buttonOK.removeClass('disabled');
				},
				cb_img_start = function() {
					$("#mce_upload").after("<div id='spin_container'><div id='spinner'></div></div>");
					$(".mce-primary").addClass("disabled");
					$("#widthField").val('100%');
					buttonOK.addClass('disabled');
				},
				cb_img_progress = function(file) {},
				cb_img_error = function() {
					alert('Upload error');
				};

				InitUploadImgEx("hidden", "mce_upload", "hidden",
					cb_img_url, cb_img_error, cb_img_start, cb_img_progress);
		}
		...
		if (imageDimensions) {
			generalFormItems.push({
				...
				spacing: 5,
				items: [
					{name: 'width', id: "widthField", type: 'textbox', maxLength: 5, size: 3, onchange: recalcSize, ariaLabel: 'Width'},
					{type: 'label', text: 'x'},
					{name: 'height', id: "heightField", type: 'textbox', maxLength: 5, size: 3, onchange: recalcSize, ariaLabel: 'Height'},
